...
| 문서 | 무엇의 근거인가 |
| --- | --- |
| [기능명세서.md](./기능명세서.md) | 기능 요구사항. 수치·문구는 여기가 유일한 출처 |
...
3. 기능명세서 [부록 B](./기능명세서.md#부록-b-명세-결손-목록)의 결손 항목을 구현해야 하는 경우
...
| `auth/**`, `auth/api/**` | **김연호 구현 · 황정빈 리뷰. 계약 변경 시 즉시 공유** |
...
공통 계약·인가·baseline처럼 두 라인에 동시에 영향을 주는 토대는 함께 확정합니다. 다만 병목을 줄이기 위해 **하네스는 황정빈, 인증·배포는 김연호가 선행 담당**합니다. 여기서 책임자를 정하는 것은 독립 설계를 허용한다는 뜻이 아니며, 공통 규칙 변경은 반드시 상대 리뷰를 거칩니다.
...
**0-2의 `common/time`과 `common/test`는 김연호가 작성합니다.** (`common`은 황정빈 주도이지만 이 두 경로는 예외) 김연호의 첫 과제가 쿨다운·스트릭이라 고정 Clock 도구를 직접 만들어두는 편이 낫습니다.

**0-3 `auth`는 김연호가 다른 기능 라인보다 먼저 착수할 수 있습니다.** 인증 스텁/사용자 기반이 필요한 테스트의 병목을 줄이기 위한 예외이며, 계약이나 공통 보안 규칙을 바꾸는 경우 황정빈 리뷰 후 머지합니다.

**0-7 배포는 김연호가 담당합니다.** 다만 배포 방식·인프라 선택이 근거 문서에 확정되어 있지 않은 부분은 임의로 결정하지 않고 §1.2 규칙에 따라 질문합니다.
...
| **참조** | [기능명세서 §4.4 정량](./기능명세서.md#4-인지-훈련-cist-정량-명세) |
...
| **참고** | `RPT-SUM-002`·`RPT-COG-004`는 존재하지 않는 ID ([부록 B](./기능명세서.md#부록-b-명세-결손-목록)). 각각 `RPT-LST-002`·`RPT-ATT-004`로 해석 |
...
| 단계 | 황정빈 | 김연호 | 동기화 지점 |
| --- | --- | --- | --- |
| Phase 0 선행 | **하네스·`BACKLOG.md` 정리** + 공통 토대 | **`auth` 선행 개발 + 배포 기반 구성** + 공통 토대 | 공통 계약·인가·baseline은 함께 확인 |
...
| 6 | 함께 — `platform/notification`, 통합 테스트, QA | 배포 최종 검증 | 릴리스 전 하네스·백로그와 실제 상태 일치 확인 |
...
| **김연호가 시작하자마자 어려운 걸 만남** | 쿨다운·재투입이 첫 과제. `common/time`·`common/test`를 김연호가 Phase 0에서 직접 만들어 대비 |
| **두 에이전트가 같은 가정을 다르게 함** | §1.2 — 추측 금지, 멈추고 질문 |
| **하네스와 실제 작업 상태가 어긋남** | 황정빈이 하네스를 관리하되, 각 담당자는 PR과 동시에 자기 `BACKLOG.md` 카드를 갱신 |
| **인증이 늦어 스텁 테스트가 막힘** | 김연호가 `auth`를 Phase 0 선행 작업으로 먼저 진행하고, 계약 변경만 황정빈과 동기화 |
| **배포 설정이 막판에 몰림** | 김연호가 Phase 0부터 배포 기반을 잡고 단계 6에서 최종 검증. 미확정 인프라는 임의 결정 금지 |
| **Flyway 충돌** | §1.4 대역 분리 |