...
>
> 이 문서는 소유권·병렬 작업 계획의 기준이다. 병합 완료 여부는 [v2-backlog.md](./v2-backlog.md)를 따른다.
...
| 이후 신규 마이그레이션 | `origin/main`의 최대 버전 다음 미사용 번호를 사용하고, 브랜치 리베이스 뒤 중복을 다시 확인 |

개인별 고정 대역은 실제 병합 이력(`V118`~`V128`)과 맞지 않아 폐기한다. 이미 머지된 마이그레이션은 **수정하지 않고** 새 번호를 추가하며, 충돌한 브랜치는 리베이스 후 파일 번호를 변경한다.
...
**0-7 배포는 황정빈이 담당합니다.** Docker·EC2 배포 기반은 구성되어 있으며, 배포 워크플로가 운영 비밀값(`S3_BUCKET` 포함)을 먼저 검증하고 EC2의 actuator 헬스체크까지 확인합니다. 메일·Gemini의 실제 외부 연동은 별도 운영 점검이 필요합니다.
...
| **핵심 수치** | 영역별 정답률 **≥70% 🟢 / 40~70% 🟡 / <40% 또는 4주 연속 하락 🟠** — 4주 하락은 모든 주 자동 채점 + 매주 엄격 하락만 인정(결측·동률 제외) · 출석 주 5일↑🟢 / 3~4일🟡 / 2일↓🟠 · 표시창 최근 7일 + 4주 |
